...
async function processJob(masterId: number, key: string, workerId: string, workerIdx: number) {
  ...
  try {
    ...
    // Fix progress calculation: use filtered_emails count (Clean count) instead of master_emails count (Upload count)
    const totalQ = await query<{ count: string }>('SELECT COUNT(*) FROM filtered_emails fe JOIN master_emails me ON fe.master_id=me.id WHERE me.batch_id=$1 AND fe.status NOT LIKE \'removed:%\'', [batchId])
